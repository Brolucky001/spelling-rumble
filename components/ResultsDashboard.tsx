...
export function ResultsDashboard({ role, userId }: ResultsDashboardProps) {
  ...
  useEffect(() => { const path = role === "student" ? "/api/results/me" : role === "school" ? `/api/schools/${userId}/report` : "/api/admin/overview"; void get(path).then(setData).catch((reason) => setError(reason instanceof Error ? reason.message : "Unable to load dashboard.")); }, [role, userId]);
  ...
  if (Array.isArray(data)) return <section className="mt-6"><h2 className="text-2xl font-black">Official results</h2>{data.length ? <div className="mt-3 grid gap-3">{data.map((result) => { const item = result as { id: string; score: number; accuracy: number; totalQuestions: number }; return <div key={item.id} className="rounded-xl bg-primary-50 p-4 dark:bg-slate-900"><span className="font-black">{item.score} XP</span><span className="ml-3 text-sm">{item.accuracy}% accuracy · {item.totalQuestions} questions</span></div>; })}</div> : <p className="mt-3 text-slate-500">No official results yet.</p>}</section>;
