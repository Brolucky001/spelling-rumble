...
function IntegrityMonitor({ competitionId }: { competitionId: string }) {
  const video = useRef<HTMLVideoElement>(null); const stream = useRef<MediaStream | null>(null); const fullscreenSeen = useRef(false); const [status, setStatus] = useState("Requesting camera access…");
  useEffect(() => {
    let active = true;
...
function AdminCompetitionForm({ onCreated, notice }: { onCreated: (id: string) => void; notice: string }) {
  const [title, setTitle] = useState(""); const [customId, setCustomId] = useState(""); const [difficulty, setDifficulty] = useState<Difficulty>("Beginner"); const [questionIds, setQuestionIds] = useState(""); const [registrationFee, setRegistrationFee] = useState("0"); const [startsAt, setStartsAt] = useState(""); const [endsAt, setEndsAt] = useState(""); const [proctoringEnabled, setProctoringEnabled] = useState(false); const [directory, setDirectory] = useState<CompetitionDirectory>({ schools: [], students: [] }); const [history, setHistory] = useState<CompetitionHistory[]>([]); const [selectedSchools, setSelectedSchools] = useState<string[]>([]); const [selectedStudents, setSelectedStudents] = useState<string[]>([]); const [error, setError] = useState("");
