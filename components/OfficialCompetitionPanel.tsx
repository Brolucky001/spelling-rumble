...
async function api<T>(path: string, body?: unknown, method?: "POST" | "PATCH" | "DELETE"): Promise<T> {
  const token = await auth.currentUser?.getIdToken();
  if (!token) throw new Error("Sign in to continue.");
  let response: Response;
  try {
    response = await fetch(path, { method: method ?? (body ? "POST" : "GET"), headers: { Authorization: `Bearer ${token}`, ...(body ? { "Content-Type": "application/json" } : {}) }, body: body ? JSON.stringify(body) : undefined });
  } catch {
...
function IntegrityMonitor({ competitionId }: { competitionId: string }) {
  const video = useRef<HTMLVideoElement>(null); const stream = useRef<MediaStream | null>(null); const fullscreenSeen = useRef(false); const [status, setStatus] = useState("Requesting camera accessÃ¢â‚¬Â¦");
  useEffect(() => {
    let active = true;
...
type CompetitionHistory = { id: string; title: string; difficulty: Difficulty; status: string; startsAt: string; endsAt: string; questionIds: number[]; registrationFee?: number; maxReplays: number; timeLimitSeconds: number; proctoringEnabled?: boolean; eligibleStudentIds?: string[]; eligibleSchoolIds?: string[]; results: { id: string; studentName?: string; userId?: string; score?: number; accuracy?: number; submittedAt?: string | null; answers?: { questionId: number; response?: string; correct: boolean }[] }[]; integrityAttempts: { userId: string; status: string; integrityEventCount: number; lastIntegrityEvent: string | null; lastIntegrityEventAt: string | null }[]; };

function AdminCompetitionForm({ onCreated, notice }: { onCreated: (id: string) => void; notice: string }) {
  const [title, setTitle] = useState(""); const [customId, setCustomId] = useState(""); const [difficulty, setDifficulty] = useState<Difficulty>("Beginner"); const [questionIds, setQuestionIds] = useState(""); const [registrationFee, setRegistrationFee] = useState("0"); const [startsAt, setStartsAt] = useState(""); const [endsAt, setEndsAt] = useState(""); const [maxReplays, setMaxReplays] = useState("2"); const [timeLimitMinutes, setTimeLimitMinutes] = useState("10"); const [proctoringEnabled, setProctoringEnabled] = useState(false); const [directory, setDirectory] = useState<CompetitionDirectory>({ schools: [], students: [] }); const [history, setHistory] = useState<CompetitionHistory[]>([]); const [selectedSchools, setSelectedSchools] = useState<string[]>([]); const [selectedStudents, setSelectedStudents] = useState<string[]>([]); const [error, setError] = useState(""); const [message, setMessage] = useState(""); const [createOpen, setCreateOpen] = useState(true);
  ...
  async function publish(id: string) { try { setError(""); await api("/api/competitions/" + encodeURIComponent(id) + "/publish", {}); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to publish competition."); } }
  async function edit(competition: CompetitionHistory) {
    const nextTitle = window.prompt("Competition title", competition.title); if (nextTitle === null) return;
    const nextQuestionIds = window.prompt("Question IDs, separated by commas", competition.questionIds.join(",")); if (nextQuestionIds === null) return;
    const nextFee = window.prompt("Registration fee (NGN)", String(competition.registrationFee ?? 0)); if (nextFee === null) return;
    const nextStartsAt = window.prompt("Start date and time (YYYY-MM-DDTHH:MM)", competition.startsAt.slice(0, 16)); if (nextStartsAt === null) return;
    const nextEndsAt = window.prompt("End date and time (YYYY-MM-DDTHH:MM)", competition.endsAt.slice(0, 16)); if (nextEndsAt === null) return;
    const nextMaxReplays = window.prompt("Maximum replays per question", String(competition.maxReplays)); if (nextMaxReplays === null) return;
    const nextMinutes = window.prompt("Official time limit (minutes)", String(Math.round(competition.timeLimitSeconds / 60))); if (nextMinutes === null) return;
    try { setError(""); await api("/api/competitions/" + encodeURIComponent(competition.id), { title: nextTitle.trim(), difficulty: competition.difficulty, startsAt: new Date(nextStartsAt).toISOString(), endsAt: new Date(nextEndsAt).toISOString(), questionIds: nextQuestionIds.split(",").map(Number).filter(Number.isInteger), registrationFee: Number(nextFee), maxReplays: Number(nextMaxReplays), timeLimitSeconds: Math.round(Number(nextMinutes) * 60), proctoringEnabled: Boolean(competition.proctoringEnabled), eligibleStudentIds: competition.eligibleStudentIds ?? [], eligibleSchoolIds: competition.eligibleSchoolIds ?? [] }, "PATCH"); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update competition."); }
  }
  async function editPastTitle(competition: CompetitionHistory) {
    const nextTitle = window.prompt("Competition title", competition.title); if (nextTitle === null) return;
    try { setError(""); setMessage(""); await api("/api/competitions/" + encodeURIComponent(competition.id), { title: nextTitle.trim() }, "PATCH"); await load(); setMessage("Competition title updated."); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update competition title."); }
  }
  function copySetup(competition: CompetitionHistory) {
    setCustomId(""); setTitle(competition.title + " (Copy)"); setDifficulty(competition.difficulty); setQuestionIds(competition.questionIds.join(",")); setRegistrationFee(String(competition.registrationFee ?? 0)); setMaxReplays(String(competition.maxReplays)); setTimeLimitMinutes(String(Math.round(competition.timeLimitSeconds / 60))); setProctoringEnabled(Boolean(competition.proctoringEnabled)); setSelectedSchools(competition.eligibleSchoolIds ?? []); setSelectedStudents(competition.eligibleStudentIds ?? []); setStartsAt(""); setEndsAt(""); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  async function remove(competition: CompetitionHistory) {
    if (!window.confirm("Delete " + competition.title + " and all of its saved attempts, results, and rankings? This cannot be undone.")) return;
    try { setError(""); await api("/api/competitions/" + encodeURIComponent(competition.id), undefined, "DELETE"); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to delete competition."); }
  }
  return <section className="mx-auto w-full max-w-5xl rounded-[1.75rem] border border-primary-100 bg-white p-6 shadow-soft dark:border-slate-700 dark:bg-slate-800"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-black tracking-widest text-primary-600">COMPETITION ADMINISTRATION</p><h1 className="mt-1 text-3xl font-black text-slate-950 dark:text-white">Competition management</h1></div><button type="button" onClick={() => setCreateOpen((open) => !open)} className="rounded-lg border border-primary-200 px-4 py-2 text-sm font-black text-primary-700">{createOpen ? "Hide create form" : "Create competition"}</button></div><form onSubmit={submit} className={createOpen ? "mt-6 grid gap-4" : "hidden"}><input value={customId} onChange={(event) => setCustomId(event.target.value)} className="rounded-lg border p-3 text-slate-950" placeholder="Custom competition ID (optional, e.g. 2026-finals)" /><input required value={title} onChange={(event) => setTitle(event.target.value)} className="rounded-lg border p-3 text-slate-950" placeholder="Competition title" /><select value={difficulty} onChange={(event) => setDifficulty(event.target.value as Difficulty)} className="rounded-lg border p-3 text-slate-950">{(["Beginner","Easy","Medium","Hard","Professional","Expert"] as Difficulty[]).map((item) => <option key={item}>{item}</option>)}</select><input required value={questionIds} onChange={(event) => setQuestionIds(event.target.value)} className="rounded-lg border p-3 text-slate-950" placeholder="Question IDs, e.g. 1,2,3,4,5" /><label className="font-bold">Registration fee (NGN)<input min="0" type="number" value={registrationFee} onChange={(event) => setRegistrationFee(event.target.value)} className="mt-1 w-full rounded-lg border p-3 text-slate-950" /></label><label className="font-bold">Starts at<input required type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} className="mt-1 w-full rounded-lg border p-3 text-slate-950" /></label><label className="font-bold">Ends at<input required type="datetime-local" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} className="mt-1 w-full rounded-lg border p-3 text-slate-950" /></label><label className="font-bold">Maximum replays per question<input required min="0" step="1" type="number" value={maxReplays} onChange={(event) => setMaxReplays(event.target.value)} className="mt-1 w-full rounded-lg border p-3 text-slate-950" /></label><label className="font-bold">Official time limit (minutes)<input required min="1" type="number" value={timeLimitMinutes} onChange={(event) => setTimeLimitMinutes(event.target.value)} className="mt-1 w-full rounded-lg border p-3 text-slate-950" /></label><label className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><input type="checkbox" checked={proctoringEnabled} onChange={(event) => setProctoringEnabled(event.target.checked)} /><span><strong>Enable competition integrity monitoring</strong><span className="block">Requests camera access, displays a timer, and records focus/fullscreen flags for review. Video is not recorded or uploaded.</span></span></label><PagedAssignmentList title="Eligible schools" description="School IDs are shown below. Selecting a school allows its assigned students to participate." rows={directory.schools.map((school) => ({ id: school.id, label: school.name, detail: `${school.status} Ã‚Â· ${school.id}` }))} selected={selectedSchools} onToggle={(id) => toggle(id, selectedSchools, setSelectedSchools)} /><PagedAssignmentList title="Eligible students" description="Student user IDs are shown below. Select individual students when needed." rows={directory.students.map((student) => ({ id: student.id, label: student.displayName, detail: `${student.email || "No email"} Ã‚Â· User ID: ${student.id}${student.schoolId ? ` Ã‚Â· School ID: ${student.schoolId}` : ""}` }))} selected={selectedStudents} onToggle={(id) => toggle(id, selectedStudents, setSelectedStudents)} /><button className="rounded-lg bg-primary-600 px-5 py-3 font-black text-white">Create competition</button></form>{error && <p role="alert" className="mt-3 text-red-700">{error}</p>}{message && <p role="status" className="mt-3 text-primary-700">{message}</p>}{notice && <p role="status" className="mt-3 text-primary-700">{notice}</p>}<section className="mt-8 border-t pt-6"><div className="flex items-center justify-between gap-3"><h2 className="text-2xl font-black">Past competitions, responses, and integrity flags</h2><button type="button" onClick={() => void load()} className="rounded-lg border border-primary-200 px-3 py-2 text-sm font-black text-primary-700">Refresh</button></div><div className="mt-4 grid gap-4">{history.map((competition) => <article key={competition.id} className="rounded-xl bg-primary-50 p-4 dark:bg-slate-900"><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{competition.title}</h3><span className="rounded-full bg-white px-2 py-1 text-xs font-bold text-primary-700">{competition.status}</span><code className="text-xs">ID: {competition.id}</code>{competition.status === "scheduled" && <><button type="button" onClick={() => void edit(competition)} className="rounded border border-primary-300 px-2 py-1 text-xs font-black text-primary-700">Edit</button><button type="button" onClick={() => void publish(competition.id)} className="rounded border border-primary-300 px-2 py-1 text-xs font-black text-primary-700">Publish</button></>}{competition.status !== "scheduled" && <><button type="button" onClick={() => void editPastTitle(competition)} className="rounded border border-primary-300 px-2 py-1 text-xs font-black text-primary-700">Edit title</button><button type="button" onClick={() => copySetup(competition)} className="rounded border border-primary-300 px-2 py-1 text-xs font-black text-primary-700">Copy setup</button></>}{(competition.status !== "active" || new Date(competition.endsAt).getTime() < Date.now()) && <button type="button" onClick={() => void remove(competition)} className="rounded border border-red-300 px-2 py-1 text-xs font-black text-red-700">Delete</button>}</div><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{competition.results.length} final response{competition.results.length === 1 ? "" : "s"}</p><div className="mt-3 grid gap-2">{competition.results.map((result) => <details key={result.id} className="rounded-lg bg-white p-3 dark:bg-slate-800"><summary className="cursor-pointer font-bold">{result.studentName ?? result.userId ?? "Student"} Ã¢â‚¬â€ {result.score ?? 0} points, {result.accuracy ?? 0}%</summary>{result.answers ? <ul className="mt-2 grid gap-1 text-sm">{result.answers.map((answer) => <li key={answer.questionId}>Question {answer.questionId}: Ã¢â‚¬Å“{answer.response ?? ""}Ã¢â‚¬Â Ã¢â‚¬â€ {answer.correct ? "correct" : "incorrect"}</li>)}</ul> : <p className="mt-2 text-sm text-slate-500">Response text was not stored for this earlier submission.</p>}</details>)}</div>{competition.integrityAttempts.length > 0 && <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950"><strong>Integrity flags for review</strong>{competition.integrityAttempts.map((attempt) => <p key={attempt.userId} className="mt-1 break-all">User ID {attempt.userId}: {attempt.integrityEventCount} flag(s), latest: {attempt.lastIntegrityEvent ?? "unknown"}</p>)}</div>}</article>)}{!history.length && <p className="text-sm text-slate-500">No competitions have been created yet.</p>}</div></section></section>;
}
