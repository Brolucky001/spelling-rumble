...
function AdminCompetitionForm({ onCreated, notice }: { onCreated: (id: string) => void; notice: string }) {
  ...
  async function edit(competition: CompetitionHistory) {
    const title = window.prompt("Competition title", competition.title); if (title === null) return;
    const questionIds = window.prompt("Question IDs, separated by commas", competition.questionIds.join(",")); if (questionIds === null) return;
    const fee = window.prompt("Registration fee (NGN)", String(competition.registrationFee ?? 0)); if (fee === null) return;
    const startsAt = window.prompt("Start date and time (YYYY-MM-DDTHH:MM)", competition.startsAt.slice(0, 16)); if (startsAt === null) return;
    const endsAt = window.prompt("End date and time (YYYY-MM-DDTHH:MM)", competition.endsAt.slice(0, 16)); if (endsAt === null) return;
    const maxReplays = window.prompt("Maximum replays per question", String(competition.maxReplays)); if (maxReplays === null) return;
    const minutes = window.prompt("Official time limit (minutes)", String(Math.round(competition.timeLimitSeconds / 60))); if (minutes === null) return;
    try {
      setError("");
      await api("/api/competitions/" + encodeURIComponent(competition.id), { title: title.trim(), difficulty: competition.difficulty, startsAt: new Date(startsAt).toISOString(), endsAt: new Date(endsAt).toISOString(), questionIds: questionIds.split(",").map(Number).filter(Number.isInteger), registrationFee: Number(fee), maxReplays: Number(maxReplays), timeLimitSeconds: Math.round(Number(minutes) * 60), proctoringEnabled: Boolean(competition.proctoringEnabled), eligibleStudentIds: competition.eligibleStudentIds ?? [], eligibleSchoolIds: competition.eligibleSchoolIds ?? [] }, "PATCH");
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update competition."); }
  }
