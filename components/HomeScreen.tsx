...
export function HomeScreen({ studentName, difficulty, sessionLength, results, onDeleteHistory, onStudentNameChange, onDifficultyChange, onSessionLengthChange, onStart }: HomeScreenProps) {
  ...
  const nextBadge = achievements.find((achievement) => !achievement.unlocked);
  return <div className="animate-fade-in pb-10">
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <aside className="grid gap-6"><div className="rounded-[1.75rem] bg-[#1D1620] p-6 text-white shadow-soft"><p className="text-xs font-black tracking-widest text-gold-400">YOUR PROGRESS</p><div className="mt-5 grid grid-cols-2 gap-5"><Metric value={total.toString()} label="Games played" /><Metric value={`${accuracy}%`} label="Avg. accuracy" /><Metric value={totalScore.toString()} label="Total score" /><Metric value={`${achievements.filter((achievement) => achievement.unlocked).length}/${achievements.length}`} label="Badges earned" /></div>{nextBadge ? <div className="mt-6 rounded-xl bg-white/10 p-4"><div className="flex justify-between gap-3 text-sm font-bold"><span>Next badge</span><span className="text-right text-gold-400">{nextBadge.label}</span></div><p className="mt-1 text-xs text-white/70">{nextBadge.description}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gold-500" style={{ width: `${Math.round((nextBadge.progress / nextBadge.target) * 100)}%` }} /></div></div> : <div className="mt-6 rounded-xl bg-gold-500/20 p-4 text-sm font-black text-gold-400">All practice badges unlocked!</div>}<div className="mt-5 grid gap-2">{achievements.map((achievement) => <div key={achievement.id} className={`rounded-lg px-3 py-2 text-sm font-bold ${achievement.unlocked ? "bg-gold-500 text-primary-950" : "bg-white/10 text-white/55"}`}><span>{achievement.unlocked ? "Unlocked" : "Locked"}: </span>{achievement.label}</div>)}</div></div><div className="rounded-[1.75rem] border border-primary-100 bg-primary-50 p-6 dark:border-slate-700 dark:bg-slate-800"><p className="text-sm font-black text-primary-600">HOW IT WORKS</p><ol className="mt-4 grid gap-4 text-sm font-bold text-slate-700 dark:text-slate-200"><li><span className="mr-3 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary-600 text-white">1</span>Listen closely to the sentence</li><li><span className="mr-3 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary-600 text-white">2</span>Type exactly what you hear</li><li><span className="mr-3 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary-600 text-white">3</span>Earn scores and badges</li></ol></div></aside>
    </section>
